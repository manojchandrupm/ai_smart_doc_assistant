...
# If True  → stream ends without a Sources section
# If False → Sources section is appended after the answer
# ─────────────────────────────────────────────────────────
GENERAL_KEYWORDS = [
...
async def query_document(payload: QueryRequest):
    ...
    try:
        ...
        # Only include sources for document-related questions
...
async def query_document_stream(payload: QueryRequest):
    ...
    try:
        ...
        async def event_generator():
            ...
            # Only append sources for document-related questions
            if not general:
                yield "\n\nSources:\n"
                seen = set()
                for m in matches:
                    if m["filename"] not in seen:
                        seen.add(m["filename"])
                        yield f"- {m['filename']}\n"
            # For greetings: stream ends here with no sources appended
